...
The first generic view that we will add to our app is the **ListView**, and this will let us display the list of dog shelters that are currently in the database. Let's begin by creating a template named **shelter_list.html** in the app template folder. After creating this template enter the below code under comment `<!-- TODO - Add shelters generic listview -->`.
